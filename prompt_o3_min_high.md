## ロール
あなたは一流のロボティクス・機械学習の研究者として振舞います

## 作業
添付の参考実装を踏まえて，物理シミュレータGenesisとrsl-rlパッケージのPO実装を用いて獲得した歩容を，unitree_sdk2_pythonを使って実機へデプロイするコードを作成する．
あいまいな一般論に基づいて考えるのではなく，与えられた実装を組み合わせて目標を達成することを最優先に行う．
平坦な床で eval .py で step() 関数の target_dof_pos 目標指示角だけ取り出して実機に送信する一方通行でもよいです．

## 背景
物理シミュレータGenesisを用いて強化学習で獲得した歩容をgo2実機にdeployしたい

## 参考実装

### unitree_sdk2_pythonのlow-level参考実装

unitree_sdk2_pythonが提供するlow-level制御（関節の角度やトルクなどを直接指定できるAPI）をgo2向けの開発で利用するサンプルコード．
あなたはこれベースにGenesisの学習結果を送信するコードを作成すること．
...
Genesisnで学習した結果を読み込んで3D描画するコード．学習済みの歩容を読み出す処理などはここから参考にすること．
...
Genesisnで学習する際の環境や観測を記述している．
学習データで実機を歩かせるとしたら、ここらへんの obs 実機の観測データを毎フレーム入れれば良さそう．
...
genesisで学習を行ったときのコード．詳細なパラメータや設定が必要になったらこれを参照すること．
...
### unitree_rl_gym_deploy.py

unitree_gymの結果を人形実機へデプロイするコード．実装の作法やこれまでのサンプル実装に無い工夫点があればここから参考にすること．
